...
function build-one {
  local ATTRPATH=$1
  nix-build -o "$OUTDIR/$ATTRPATH-build" nix/generic/package-instance.nix -Q -A "\"$ATTRPATH\".allComponents"\
    2> >(tee "$OUTDIR/$ATTRPATH-build.txt" >&2)
  (($? == 0)) || { echo "$ATTRPATH: all-component build failed" >> "$SUMMARY"; return 1; }
  nix-build -o "$OUTDIR/$ATTRPATH-test-result" -Q nix/generic/package-instance.nix -A "\"$ATTRPATH\".$PACKAGENAME.checks.tests"
...
function cabal-check {
  nix-build --no-out-link nix/generic/package-instance.nix -A "\"$CABAL_CHECK_ATTRPATH\".cabal-check"\
...
CLEANEDSOURCE=$(nix-instantiate --eval --read-write-mode nix/generic/package-instance.nix -A "cleanedSource.outPath")
(($? == 0)) || exit 1
