...
CABAL_FILE_PATH=$(nix-instantiate --read-write-mode --eval --arg base-config "$SEAAYE_CONFIG_NIX" --arg location "$SEAAYE_LOCATION" -A "cabal-file-path" "$SEAAYE_LIBDIR/main.nix" --json | jq -r)
